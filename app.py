...
st.markdown("""
    <div class="upload-section">
        ...
        <p style="text-align: center; font-size: 1.1rem; margin-bottom: 2rem;">
            Upload your CSV file for mass sentiment analysis
        </p>
    </div>
""", unsafe_allow_html=True)

uploaded_file = st.file_uploader(
    "Choose a CSV file with 'text' column",
    type=["csv"],
    help="Your CSV should have a column named 'text' containing the text to analyze"
)
    ...
    try:
        ...
        if 'text' not in df.columns:
            ...
            with st.spinner('🔮 Analyzing all texts...'):
                ...
                df['confidence'] = model.predict_proba(df['clean_text']).max(axis=1) * 100
                ...
                # Style the dataframe
                styled_df = df[['text', 'sentiment', 'confidence']].style.format({
                    'confidence': '{:.2f}%'
                }).background_gradient(subset=['confidence'], cmap='viridis')
